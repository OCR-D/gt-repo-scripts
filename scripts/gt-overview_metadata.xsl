            ...
            <xsl:if test="$docMETADATA//fn:map/fn:string[@key='gtTyp'] = 'data_structure_and_text'">structure_text_profile.json</xsl:if>
                ...
                <xsl:for-each select="$holeMetric/array/array">
                    ...
                    <xsl:if test="$Image1 != ''">
                        <xsl:if test="$Image2 = $Page">
                            cd <xsl:value-of select="substring-after(substring-before(map/@file, 'GT-PAGE'), 'file:')"/> \
                            page-xml-draw -i <xsl:value-of select="substring-after(map/@file, 'file:')"/> -o ghout/drawImg<xsl:value-of select="substring-after(map/@file, 'file:')"/>.png -b GT-PAGE/<xsl:value-of select="map/image2"/> --profile <xsl:value-of select="substring-before(substring-after(map/@file, 'file:'),'data')"/>scripts/<xsl:value-of select="$gtTypeJson"/>
                        </xsl:if>
                    </xsl:if>
