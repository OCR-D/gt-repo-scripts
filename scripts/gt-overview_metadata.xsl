                                        ...
                                        <xsl:if test="$docMETADATA//fn:array[@key='authors']/fn:map/fn:array[@key='roles'] !=''">
                                            <mods:role>
                                                <xsl:for-each select="$docMETADATA//fn:array[@key='authors']/fn:map/fn:array[@key='roles']/fn:string">
                                                    <mods:roleTerm authority="ocrdrelator" type="code" valueURI="https://raw.githubusercontent.com/tboenig/gt-metadata/master/schema/2023-10-25/schema.json"><xsl:value-of select="."/></mods:roleTerm>
                                                </xsl:for-each>
                                            </mods:role>
                                        </xsl:if>
                                    </mods:name>
                        ...
                        <mets:mets
                            ...
                            <xsl:variable name="t4">
                                <xsl:copy-of copy-namespaces="no" select="$t1//mets:metsHdr
                                    | $t1//mets:dmdSec 
                                    | $t1//mets:amdSec" />
                                <mets:fileSec><xsl:copy-of copy-namespaces="no" select="$t1//mets:fileSec/child::*[@USE  != 'DEFAULT']"/></mets:fileSec>
...
abstract: <xsl:value-of select="normalize-space($docMETADATA//fn:map/fn:string[@key='description'])"/>
keywords:
  - ocr-d
  - repository
  - segmentation
